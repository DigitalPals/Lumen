//! Registers the Lumen icon directory with GTK's IconTheme so icons are discoverable at runtime.
//!
...
const SYSTEM_ICONS_PATH: &str = "/usr/share/lumen/icons";

const INDEX_THEME_CONTENT: &str = r#"[Icon Theme]
Name=Lumen Icons
Comment=Icons installed by Lumen
...
/// Manages GTK IconTheme registration for Lumen icons.
///
/// [`IconRegistry::init`] should be invoked at application startup to ensure
/// GTK can discover icons installed via `lumen icons install`.
#[derive(Debug, Clone)]
...
impl IconRegistry {
    ...
    ///
    /// The default path is `~/.local/share/lumen/icons/`.
    ///
    /// # Errors
    ///
    ...
    ///
    /// Uses `$XDG_DATA_HOME/lumen/icons` or `~/.local/share/lumen/icons`.
    ///
    /// # Errors
    ///
    /// Returns error if `$HOME` is not set and `$XDG_DATA_HOME` is also unset.
    pub fn default_path() -> Result<PathBuf> {
        ...
        Ok(data_home.join("lumen").join("icons"))
    }
    ...
    pub(crate) fn system_icon_paths() -> Vec<PathBuf> {
        ...
        for dir in xdg_dirs.split(':').filter(|d| !d.is_empty()) {
            let path = PathBuf::from(dir).join("lumen/icons");
            if path.exists() {
                paths.push(path);
            }
        }
