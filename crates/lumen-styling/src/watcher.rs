...
//!
//! [`spawn`] watches `~/.config/lumen/styles/` recursively and rebuilds the
...
use lumen_config::{Config, ConfigService, infrastructure::paths::ConfigPaths};
...
/// component.
///
/// Idempotently scaffolds `~/.config/lumen/styles/index.scss` if missing.
pub fn spawn<C>(
    ...
    C::CommandOutput: Send + 'static,
{
    ...
    let Some(styles_dir) = user_styles_dir(&config_dir) else {
        info!("no ~/.config/lumen/styles/ directory; user style watcher idle");
        return;
    };
