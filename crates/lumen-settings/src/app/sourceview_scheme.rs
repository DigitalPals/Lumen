...
use lumen_config::schemas::styling::PaletteConfig;
use tracing::warn;

pub(crate) const SCHEME_ID: &str = "lumen";
const SCHEME_FILENAME: &str = "lumen.xml";

static SCHEME_DIR_REGISTERED: Mutex<bool> = Mutex::new(false);

fn scheme_dir() -> PathBuf {
    if let Some(runtime_dir) = env::var_os("XDG_RUNTIME_DIR") {
        return PathBuf::from(runtime_dir).join("lumen-sourceview");
    }

    let user = env::var("USER").unwrap_or_else(|_| String::from("unknown"));
    env::temp_dir().join(format!("lumen-sourceview-{user}"))
}

pub(crate) fn update_lumen_scheme(palette: &PaletteConfig) {
    let dir = scheme_dir();
...
<style-scheme id="{SCHEME_ID}" name="Lumen" version="1.0">
