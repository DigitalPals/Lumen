...
use lumen_config::schemas::styling::{ThemeProvider, ThresholdColors};
...
impl InlineStyling for BarButton {
    ...
    fn build_css(&self) -> String {
        let is_lumen = matches!(self.settings.theme_provider.get(), ThemeProvider::Lumen);
        let t = &self.threshold_overrides;

        let icon_color =
            ThresholdColors::resolve_or(&t.icon_color, self.resolve_icon_color(is_lumen));
        let label_color = ThresholdColors::resolve_or(
            &t.label_color,
            resolve_color(&self.colors.label_color, is_lumen),
        );
        let icon_bg = ThresholdColors::resolve_or(
            &t.icon_background,
            resolve_color(&self.colors.icon_background, is_lumen),
        );
        let button_bg = ThresholdColors::resolve_or(
            &t.button_background,
            resolve_color(&self.colors.button_background, is_lumen),
        );
        let border_color = ThresholdColors::resolve_or(
            &t.border_color,
            resolve_color(&self.colors.border_color, is_lumen),
        );
