...
impl BarButton {
    ...
    pub(super) fn resolve_icon_color(&self, is_lumen_themed: bool) -> Cow<'static, str> {
        let color = match self.colors.icon_color.get() {
            ColorValue::Custom(_) if !is_lumen_themed => self.colors.icon_color.default().clone(),
            other => other,
        };
