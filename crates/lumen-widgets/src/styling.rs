//! Inline CSS styling traits and helpers for Lumen components.
//!
...
use gtk4 as gtk;
use lumen_config::{ConfigProperty, schemas::styling::ColorValue};
...
///
/// `Custom(hex)` is used as-is under Lumen's built-in palette. Under a dynamic
/// provider it falls back to the field's default value, since a fixed hex has
/// no meaningful mapping to a generated palette.
pub fn resolve_color(prop: &ConfigProperty<ColorValue>, is_lumen_theme: bool) -> Cow<'static, str> {
    match prop.get() {
        ColorValue::Custom(_) if !is_lumen_theme => prop.default().to_css(),
        other => other.to_css(),
    }
}
