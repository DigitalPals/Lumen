...
//! a single `lumen-shell.ftl` per locale directory, and enforces link order
...
fn main() {
    ...
    // lumen-idle-inhibit can pull in libwayland-client early due to linker behavior.
...
fn concatenate_partials(locale_dir: &Path) {
    ...
    let output = locale_dir.join("lumen-shell.ftl");
