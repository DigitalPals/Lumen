//! Shell IPC service exposing `com.lumen.Shell1` on the session bus.
//!
...
pub use error::Error;
use lumen_ipc::shell_ipc::{SERVICE_NAME, SERVICE_PATH};
...
/// Registers the `com.lumen.Shell1` D-Bus interface and holds the
...
impl ShellIpcService {
    /// Connects to the session bus and registers the `com.lumen.Shell1` interface.
    ///
    /// # Errors
    ///
