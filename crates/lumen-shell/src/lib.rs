//! Lumen desktop shell - a GTK4/Relm4 status bar for Wayland compositors.
...
pub fn run() -> Result<(), Box<dyn Error>> {
    ...
    tracing_init::init()?;
    info!("Lumen shell starting");

    let runtime = Runtime::new()?;
    let _guard = runtime.enter();

    if runtime.block_on(bootstrap::is_already_running()) {
        eprintln!("Lumen shell is already running");
        return Ok(());
    }
