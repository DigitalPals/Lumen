...
use lumen_config::{infrastructure::paths::ConfigPaths, schemas::styling::ThemeProvider};
use lumen_styling::{STATIC_CSS, theme_css, user_css};
use lumen_widgets::{watch, watchers::changes_stream};
...
pub fn spawn(sender: &ComponentSender<Shell>, services: &ShellServices) {
    ...
    let theme_provider_stream = config
        ...
        .theme_provider
        .watch()
        .filter(|provider| std::future::ready(*provider == ThemeProvider::Lumen));
...
pub(super) fn build_css(config: &lumen_config::Config) -> String {
