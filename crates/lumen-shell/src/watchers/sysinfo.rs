...
fn spawn_cpu_watcher(
    config: &lumen_config::schemas::modules::CpuConfig,
...
fn spawn_memory_watcher(
    config: &lumen_config::schemas::modules::RamConfig,
    sysinfo: &Arc<SysinfoService>,
) {
    let mut stream = config.poll_interval_ms.watch();
    let sysinfo = sysinfo.clone();
...
fn spawn_disk_watcher(
    config: &lumen_config::schemas::modules::StorageConfig,
    sysinfo: &Arc<SysinfoService>,
) {
    let mut stream = config.poll_interval_ms.watch();
    let sysinfo = sysinfo.clone();
...
fn spawn_network_watcher(
    config: &lumen_config::schemas::modules::NetstatConfig,
    sysinfo: &Arc<SysinfoService>,
) {
    let mut stream = config.poll_interval_ms.watch();
    let sysinfo = sysinfo.clone();
