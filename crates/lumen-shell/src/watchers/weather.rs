    ...
    tokio::spawn(async move {
        ...
        while let Some(provider) = stream.next().await {
            let kind = match provider {
                WeatherProvider::OpenMeteo => lumen_weather::WeatherProviderKind::OpenMeteo,
                WeatherProvider::VisualCrossing => {
                    lumen_weather::WeatherProviderKind::VisualCrossing
                }
                WeatherProvider::WeatherApi => lumen_weather::WeatherProviderKind::WeatherApi,
            };
    ...
    tokio::spawn(async move {
        ...
        while let Some(units) = stream.next().await {
            let units = match units {
                TemperatureUnit::Metric => lumen_weather::TemperatureUnit::Metric,
                TemperatureUnit::Imperial => lumen_weather::TemperatureUnit::Imperial,
            };
...
fn parse_location(location: &str) -> lumen_weather::LocationQuery {
    location
        .split_once(',')
        .and_then(|(lat, lon)| {
            let lat = lat.trim().parse().ok()?;
            let lon = lon.trim().parse().ok()?;
            Some(lumen_weather::LocationQuery::coords(lat, lon))
        })
        .unwrap_or_else(|| lumen_weather::LocationQuery::city(location))
}
