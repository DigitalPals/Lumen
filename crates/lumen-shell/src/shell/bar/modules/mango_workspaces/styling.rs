...
use lumen_config::{ConfigService, schemas::styling::ThemeProvider};
use lumen_widgets::{prelude::BarSettings, styling::resolve_color};
use relm4::gtk;
...
pub(super) fn apply_styling(
    ...
    let is_lumen_theme = matches!(config.styling.theme_provider.get(), ThemeProvider::Lumen);

    let active_color = resolve_color(&tags_config.active_color, is_lumen_theme);
    let occupied_color = resolve_color(&tags_config.occupied_color, is_lumen_theme);
    let empty_color = resolve_color(&tags_config.empty_color, is_lumen_theme);
    let container_bg_color = resolve_color(&tags_config.container_bg_color, is_lumen_theme);
    let border_color = resolve_color(&tags_config.border_color, is_lumen_theme);
