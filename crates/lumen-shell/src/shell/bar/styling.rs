...
use lumen_widgets::styling::{InlineStyling, resolve_color};
...
impl InlineStyling for Bar {
    ...
    fn build_css(&self) -> String {
        ...
        let is_lumen = matches!(styling.theme_provider.get(), ThemeProvider::Lumen);

        let bg = resolve_color(&bar.bg, is_lumen);
        ...
        let border_color = resolve_color(&bar.border_color, is_lumen);
        ...
        let group_bg = resolve_color(&bar.button_group_background, is_lumen);
        let group_opacity = bar.button_group_opacity.get().value();
        let group_border_color = resolve_color(&bar.button_group_border_color, is_lumen);
