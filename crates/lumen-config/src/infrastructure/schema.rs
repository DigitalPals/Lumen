...
/// Generates the JSON Schema for Lumen's root configuration.
///
...
pub fn generate_schema() -> Option<String> {
    generate_schema_for::<Config>(&format!("lumen-config-{VERSION}"))
}
...
pub fn generate_theme_schema() -> Option<String> {
    generate_schema_for::<Palette>(&format!("lumen-theme-{VERSION}"))
}
...
///
/// Writes `schema.json`, `themes/schema.json`, and `tombi.toml` to `~/.config/lumen/` if:
...
fn ensure_root_schema() -> io::Result<()> {
    ...
    let needs_update = match fs::read_to_string(&schema_path) {
        Ok(existing) => !existing.contains(&format!("lumen-config-{VERSION}")),
        Err(_) => true,
    };
...
fn ensure_theme_schema() -> io::Result<()> {
    ...
    let needs_update = match fs::read_to_string(&schema_path) {
        Ok(existing) => !existing.contains(&format!("lumen-theme-{VERSION}")),
        Err(_) => true,
    };
