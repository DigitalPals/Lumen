...
//!
//! ```rust,no_run
//! # use lumen_audio::{AudioService, types::device::{DeviceKey, DeviceType}};
//! # async fn example() -> Result<(), lumen_audio::Error> {
//! # let audio = AudioService::new().await?;
...
//!
//! ```rust,no_run
//! # use lumen_audio::{AudioService, volume::types::Volume};
//! # async fn example() -> Result<(), lumen_audio::Error> {
//! # let audio = AudioService::new().await?;
//! if let Some(device) = audio.default_output.get() {
//!     // Mute/unmute
...
//! [`Property`]: lumen_core::Property
//! [`Property<T>`]: lumen_core::Property
