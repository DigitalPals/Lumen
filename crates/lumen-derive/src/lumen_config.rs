//! `#[lumen_config]` attribute macro for config structs.
...
/// Parsed arguments from `#[lumen_config(...)]`.
struct MacroArgs {
    config_type: ConfigType,
    i18n_prefix: Option<String>,
}

/// Entry point for the `#[lumen_config]` attribute macro.
///
...
fn generate(parsed_struct: ItemStruct, args: MacroArgs) -> syn::Result<TokenStream2> {
    ...
    let struct_fields = match &parsed_struct.fields {
        ...
        _ => {
            return Err(syn::Error::new_spanned(
                &parsed_struct,
                "lumen_config only supports structs with named fields",
            ));
        }
    };
    ...
    Ok(quote! {
        ...
        #[derive(
            ...
            serde::Deserialize,
            schemars::JsonSchema,
            lumen_derive::ApplyConfigLayer,
            lumen_derive::ApplyRuntimeLayer,
            lumen_derive::ExtractRuntimeValues,
            lumen_derive::SubscribeChanges,
            lumen_derive::ResetConfigLayer,
            lumen_derive::ResetRuntimeLayer,
            lumen_derive::ClearAllRuntime,
            lumen_derive::ClearRuntimeByPath,
            lumen_derive::CommitConfigReload,
        )]
        #[serde(default)]
...
fn process_fields(fields: &FieldsNamed, i18n_prefix: Option<&str>) -> syn::Result<ProcessedFields> {
    ...
    for field in &fields.named {
        ...
        let is_lumen_skipped = field.attrs.iter().any(|attr| {
            attr.path().is_ident("lumen")
                && attr
                    .meta
                    .require_list()
        ...
        if !is_config_property && !is_lumen_skipped {
            output
                .container_fields
        ...
        let initializer = match (&default_expr, &i18n_key) {
            (Some(expr), Some(key)) => {
                quote! { #field_ident: lumen_config::ConfigProperty::with_i18n_key(#expr, #key) }
            }
            (Some(expr), None) => {
                quote! { #field_ident: lumen_config::ConfigProperty::new(#expr) }
            }
            (None, _) => {
