...
/// Returns `true` if the field has `#[lumen(skip)]`. Skipped fields are
...
pub struct LookupKey {
    /// The key.
    pub name: String,
    /// `true` for `#[lumen(deprecated_alias)]` keys; matches emit a `tracing::warn!`.
    pub deprecated: bool,
}
...
/// `#[lumen(deprecated_alias = "...")]` entries (legacy names that emit a
