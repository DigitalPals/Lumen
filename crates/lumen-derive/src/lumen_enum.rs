//! `#[lumen_enum]` attribute macro for config enums.
//!
//! Injects the standard derive set and `#[serde(rename_all = "kebab-case")]`
//! unless the user already specified one. Serde and lumen attributes on
...
pub fn lumen_enum(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let (derive_helper_attrs, other_attrs): (Vec<_>, Vec<_>) = parsed_enum
        .attrs
        .drain(..)
        .partition(|attr| attr.path().is_ident("serde") || attr.path().is_ident("lumen"));
