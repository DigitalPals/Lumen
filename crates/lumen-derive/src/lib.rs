//! Derive macros for Lumen configuration management.
...
///
/// ```ignore
/// #[lumen_config(i18n_prefix = "settings-general")]
...
pub fn lumen_config(attr: TokenStream, item: TokenStream) -> TokenStream {
    lumen_config::lumen_config(attr, item)
}
...
/// ```
#[proc_macro_attribute]
pub fn lumen_enum(attr: TokenStream, item: TokenStream) -> TokenStream {
    lumen_enum::lumen_enum(attr, item)
}
...
/// // ]
/// ```
#[proc_macro_derive(EnumVariants, attributes(lumen))]
...
/// `#[serde(alias)]` matches silently, `#[lumen(deprecated_alias)]` matches
...
///     #[lumen(deprecated_alias = "notification")]
///     pub notifications: NotificationConfig,
/// }
/// ```
#[proc_macro_derive(ApplyConfigLayer, attributes(lumen))]
...
/// by the CLI's `lumen config set` to apply individual field changes.
///
/// Same shape as `ApplyConfigLayer`, but propagates errors so invalid
/// runtime overrides get caught before they take effect.
#[proc_macro_derive(ApplyRuntimeLayer, attributes(lumen))]
...
/// }
/// ```
#[proc_macro_derive(ExtractRuntimeValues, attributes(lumen))]
...
#[proc_macro_derive(ResetConfigLayer, attributes(lumen))]
...
#[proc_macro_derive(ResetRuntimeLayer, attributes(lumen))]
...
#[proc_macro_derive(CommitConfigReload, attributes(lumen))]
...
#[proc_macro_derive(ClearAllRuntime, attributes(lumen))]
...
/// }
/// ```
#[proc_macro_derive(SubscribeChanges, attributes(lumen))]
...
/// CLI's `lumen config reset` command.
///
/// ```ignore
/// match segment {
...
/// }
/// ```
#[proc_macro_derive(ClearRuntimeByPath, attributes(lumen))]
