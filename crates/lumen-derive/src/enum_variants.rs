...
fn generate(input: &DeriveInput) -> syn::Result<TokenStream2> {
    ...
    Ok(quote! {
        impl lumen_config::EnumVariants for #enum_name {
            fn variants() -> &'static [lumen_config::EnumVariant] {
                &[#(#variant_entries),*]
            }
        }
    })
...
/// Generates the fluent key for a variant. Checks for `#[lumen(fluent_key = "...")]`
