<p align="center">
  <img src="https://raw.githubusercontent.com/lumen-rs/lumen/master/assets/lumen.svg" width="200" alt="Lumen">
</p>

# lumen-network

WiFi and wired network management via NetworkManager D-Bus.

[![Crates.io](https://img.shields.io/crates/v/lumen-network)](https://crates.io/crates/lumen-network)
[![docs.rs](https://img.shields.io/docsrs/lumen-network)](https://docs.rs/lumen-network)
[![License: MIT](https://img.shields.io/badge/license-MIT-blue.svg)](LICENSE)

## Installation

```sh
cargo add lumen-network
```

## Usage

```rust,no_run
use lumen_network::NetworkService;
use futures::StreamExt;

async fn example() -> Result<(), lumen_network::Error> {
    let net = NetworkService::new().await?;

    // wifi is None if no adapter is present
    let Some(wifi) = net.wifi.get() else {
        println!("no WiFi adapter");
        return Ok(());
    };

    println!("SSID: {:?}, signal: {:?}", wifi.ssid.get(), wifi.strength.get());

    // react to connectivity changes
    let mut stream = wifi.connectivity.watch();
    while let Some(status) = stream.next().await {
        println!("connectivity: {status:?}");
    }
    Ok(())
}
```

## License

MIT

Part of [Lumen](https://github.com/lumen-rs/lumen).
