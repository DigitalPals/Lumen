...
use lumen_traits::ServiceMonitoring;
...
use crate::{
    ...
    lumen_daemon::LumenDaemon,
    persistence::{NotificationStore, StoredNotification},
    popup_timer::PopupTimerManager,
    service::NotificationService,
    types::dbus::{LUMEN_SERVICE_NAME, LUMEN_SERVICE_PATH, SERVICE_NAME, SERVICE_PATH},
};
...
impl NotificationServiceBuilder {
    ...
    /// Enables the Lumen D-Bus daemon for CLI control.
    ///
    /// When enabled, the service registers at `com.lumen.Notifications1`,
    /// allowing CLI tools to control notifications (dismiss, toggle DND, etc.).
    pub fn with_daemon(mut self) -> Self {
        self.register_lumen_daemon = true;
        self
    }
    ...
    pub async fn build(self) -> Result<Arc<NotificationService>, Error> {
        ...
        if self.register_lumen_daemon {
            let lumen_daemon = LumenDaemon {
                service: Arc::clone(&service),
            };
            register_dbus_object(&connection, LUMEN_SERVICE_PATH, lumen_daemon).await?;
            register_dbus_name(&connection, LUMEN_SERVICE_NAME).await?;
            info!("Lumen notification extensions registered at {LUMEN_SERVICE_NAME}");
        }
