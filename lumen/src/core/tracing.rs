...
/// Supports both pretty console output and JSON output based on LUMEN_LOG_FORMAT.
///
/// # Errors
...
pub fn init() -> Result<(), Box<dyn Error>> {
    ...
    let format = env::var("LUMEN_LOG_FORMAT").unwrap_or_else(|_| String::from("pretty"));
...
/// while file output uses LUMEN_FILE_LOG level (defaults to "info").
/// File is created in the lumen logs directory.
///
/// # Errors
...
fn init_with_file_and_console_option(enable_console: bool) -> Result<(), Box<dyn Error>> {
    ...
    let file_filter = env::var("LUMEN_FILE_LOG")
        .map(EnvFilter::new)
