...
pub enum IconsCommands {
    /// Install bundled icons required by Lumen components
    Setup,
    /// Install icons from a CDN source
    #[command(after_long_help = INSTALL_HELP)]
    Install {
        /// Source name (run 'lumen icons sources' to see available sources)
        source: String,
...
const INSTALL_HELP: &str = concat!(
    ...
    "\n",
    "Run 'lumen icons sources' to see all available icon sources.\n",
    "Icons are saved to ~/.local/share/lumen/icons/ as GTK symbolic icons.",
);

const IMPORT_HELP: &str = concat!(
    styled_header!("Examples:"),
    "\n",
    "    lumen icons import ~/Downloads/my-icon.svg my-icon\n",
    "        -> cm-my-icon-symbolic\n",
    "\n",
